...
def send_async_email_resend(app, to_email, subject, html_content, retries=3, delay=5):
    """Envoi email via API Resend avec gestion anti-spam"""
    with app.app_context():
        for attempt in range(1, retries + 1):
            try:
                ...
                if not api_key:
                    current_app.logger.error("[EMAIL] ❌ RESEND_API_KEY manquante dans la config !")
                    return
                
                current_app.logger.info(
                    f"[EMAIL] Configuration API Resend : sender={sender}"
                )
                
                # Payload avec headers anti-spam et tags
                payload = {
                    "from": sender,
                    "to": [to_email],
                    "subject": subject,
                    ...
                    "tags": [
                        {"name": "category", "value": "account_notification"}
                    ]
                }
                ...
                if response.status_code == 200:
                    data = response.json()
                    current_app.logger.info(
                        f"[EMAIL] ✅ Email envoyé avec succès (ID: {data.get('id')})"
                    )
...
def send_account_activated_email(user):
    ...
    # S'assurer que frontend_url est toujours une string valide
    ...
    # Envoi asynchrone
    Thread(
        target=send_async_email_resend,
